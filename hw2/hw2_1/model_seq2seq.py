...
with tf.variable_scope("RNN_model"):
    ...
    lstm1 = tf.nn.rnn_cell.BasicLSTMCell(dim_hidden, forget_bias=1, state_is_tuple=False)
    lstm1 = tf.contrib.rnn.DropoutWrapper(lstm1, output_keep_prob=0.5)   
    lstm2 = tf.nn.rnn_cell.BasicLSTMCell(dim_hidden,forget_bias=1, state_is_tuple=False)
    ...
    U_att = tf.Variable(tf.random_uniform([dim_hidden*2, dim_hidden*2], -0.1, 0.1), name='U_attention')
    W_att = tf.Variable(tf.random_uniform([dim_hidden*2, dim_hidden*2], -0.1, 0.1), name='W_attention')
    V_att = tf.Variable(tf.random_uniform([dim_hidden*2, 1], -0.1, 0.1), name='V_attention')
    ...
    #noise = tf.random_uniform(tf.shape(video), -0.1, 0.1, dtype=tf.float32)
    #video = video + noise
    ...
    padding_state = tf.zeros([batch_size, dim_hidden*2])
    ...
    for i in range(0, video_lstm_step):
            ...
            with tf.variable_scope("LSTM2"):
                output2, state2 = lstm2(tf.concat([padding, output1, padding_state], 1), state2)

            list_h.append(state2)
    ...
    for i in range(0, caption_lstm_step):
        ...
        list_e = []
        for h in list_h:
            e = tf.tanh(tf.matmul(h, U_att) + tf.matmul(h, W_att))
            e = tf.matmul(e, V_att)
            #print(e)
            list_e.append(e)

        list_h_t = tf.transpose(tf.stack(list_h), (1,0,2))
        list_e = tf.nn.softmax(tf.concat(list_e, axis=1))

        final_h_list = []
        for kk in range(batch_size):
            alpha_1 = tf.reshape(tf.gather(list_e, kk), (1,80))
            h_1 = tf.gather(list_h_t, kk)
            final_h = tf.matmul(alpha_1, h_1)
            final_h_list.append(final_h)


        final_h_t_list = tf.squeeze(tf.stack(final_h_list), [1])

        # print(final_h_t_list)



        with tf.variable_scope("LSTM2"):
            output2, state2 = lstm2(tf.concat([current_embed, output1, final_h_t_list],1), state2)
...
loss_out = open('loss_record.txt', 'w+')
val_loss_total = []
for epoch in range(0, epochs):
    ...
    for start in range(0, len(current_train_data)-batch_size, batch_size):
        ...
        loss_out.write('epoch ' + str(epoch) + ' loss ' + str(loss_val) + str(check_condition)+'\n')
